...
def decode_watermark(signal, audio_length_second):
    ...
    if mean_result is None or info["perfectly_matched_section"] == 0:
        print("No Watermark Found")
        return
    payload = mean_result[len_start_bit:]
    payload_str = "".join([str(i) for i in payload])
    if info["perfectly_matched_section"] == 0:
        print("Warning! No block was found that exactly matched the pattern bits.")
    else:
        pass
        # print("Found %d watermarked sections" % info["perfectly_matched_section"])
